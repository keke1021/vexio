  ...
  if (isError || !data) {
    return (
      <div className="px-6 pt-8">
        <p className="text-[#475569] text-[13px]">Sesión de caja no encontrada.</p>
  ...
  return (
    <div className="px-6 pt-8 pb-16 max-w-[900px] mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link to="/cash/sessions" className="text-[#475569] hover:text-[#64748B] transition-colors text-[13px]">← Historial de cajas</Link>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          {session.notes && (
            <p className="text-[12px] text-[#475569] mt-1">Nota: {session.notes}</p>
          )}
        </div>
        {session.isOpen ? (
          ...
          {activeCurrencies.map((cur) => {
            ...
            return (
              <div key={cur} className="bg-white border border-[#E2E8F0] rounded-xl px-5 py-4"
                style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                <p className="text-[10px] font-medium text-[#475569] uppercase tracking-[0.15em] mb-3">Balance {cur}</p>
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[13px]">
                    <span className="text-[#475569]">Monto inicial</span>
                  ...
                  {b.adjustments !== 0 && (
                    <div className="flex justify-between text-[13px]">
                      <span className="text-[#475569]">Ajuste de cierre</span>
      ...
      <div>
        ...
        <div className="border border-[#E2E8F0] rounded-xl overflow-hidden bg-white"
          style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          {movements.length === 0 && (
            <p className="text-center py-10 text-[#64748B] text-[13px]">Sin movimientos en esta sesión.</p>
          )}
          {movements.length > 0 && (
                  ...
                  <tr key={m.id} className="border-b border-[#E2E8F0]">
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium ${
                        ...
                        {m.sale && <span className="text-[#64748B] font-normal">venta</span>}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-[#475569] hidden md:table-cell">{fmtDateTime(m.createdAt)}</td>
                    <td className="px-4 py-3.5 text-[#475569] hidden lg:table-cell">{m.createdBy?.name ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
